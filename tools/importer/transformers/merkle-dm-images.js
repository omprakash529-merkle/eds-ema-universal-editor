...
/**
 * Transformer: merkle.com Dynamic Media / Scene7 image carrier.
 *
 * The merkle.com source serves ~20 images from Scene7 IS/Image
 * (assets.merkle.com/is/image/merkle/... with :preset and op_* params;
 * verified in migration-work/metadata.json .images.mapping). EDS's
 * path-based image optimizer would strip the Scene7 template/rendition
 * query params, so instead of importing a raw <img src>, this transformer
 * rewrites each DM <img> into an anchor carrier that round-trips through
 * markdown intact. The companion auto-block in scripts/scripts.js rebuilds
 * responsive <picture> markup from those anchors at render time.
 *
 ...
 * carousel image cells). Rewriting imgs to anchors before parsing would leave
 * parsers with no img and produce empty cells. Running afterTransform lets
 * parsers build their cells first; we then rewrite wherever the imgs ended up.
 *
 * Helpers below are copied byte-identical from the canonical
 * dm-scene7-helpers.js (transformer subset: detectDynamicMediaUrl,

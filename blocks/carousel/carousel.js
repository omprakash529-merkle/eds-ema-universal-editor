 ...
 * (params preserved) and handles ordinary DAM paths too. Alt is derived from
 * the slide heading in the content column for accessibility.
...
function buildImageFromUrl(column, row) {
  if (column.querySelector('picture')) return; // already an image
  const url = (column.textContent || '').trim();
  if (!/^(https?:\/\/|\/|\.\/)/.test(url)) return;
  const alt = row.querySelector('h1,h2,h3,h4,h5,h6')?.textContent.trim() || '';
  const pic = createOptimizedPicture(url, alt, true, [{ width: '2000' }]);
 ...
 * - CTA links are collected into a single .carousel-slide-cta row and
...
function decorateContent(content) {
  ...
  kids.forEach((el, idx) => {
    ...
    if (headingIdx !== -1 && idx < headingIdx) {
      el.classList.add('carousel-slide-eyebrow');
    } else {
      el.classList.add('carousel-slide-desc');
    }
  });
  ...
  if (ctaParas.length) {
    ...
    ctaRow.className = 'carousel-slide-cta';
